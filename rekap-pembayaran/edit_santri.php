...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $pdo->prepare("UPDATE santri SET nama_santri = ?, alamat = ?, telepon = ?, email = ?, tanggal_lahir = ? WHERE id = ?");
    $stmt->execute([$nama, $alamat, $telepon, $email, $tanggal_lahir, $id]);
...
<body class="flex flex-col h-screen">
    ...
    <div class="flex flex-1 mt-12">
        ...
        <div class="ml-64 flex-1 p-6">
            ...
            <!-- Konten Halaman -->
            <h1 class="text-2xl font-semibold mb-6 text-center">Edit Santri</h1>
            <hr class="py-4">

            <!-- Konten Utama-->
            <div class="container max-w-2xl mx-auto mt-8 p-6 bg-white rounded-lg shadow-lg">
                <form action="" method="post" class="flex flex-wrap -mx-3 mb-6">
                    <!-- Kolom Kiri -->
                    <div class="w-full md:w-1/2 px-3 mb-6 md:mb-0">
                        <div>
                            <label for="nama_santri" class="block text-gray-700 text-sm font-medium mb-2">Nama</label>
                            <input type="text" id="nama_santri" name="nama_santri" value="<?php echo htmlspecialchars($santri['nama_santri']); ?>" class="border border-gray-300 rounded-lg px-4 py-2 w-full text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" required>
                        </div>
                        <div>
                            <label for="alamat" class="block text-gray-700 text-sm font-medium mb-2">Alamat</label>
                            <textarea id="alamat" name="alamat" class="border border-gray-300 rounded-lg px-4 py-2 w-full text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"><?php echo htmlspecialchars($santri['alamat']); ?></textarea>
                        </div>
                    </div>
                    <!-- Kolom Kanan -->
                    <div class="w-full md:w-1/2 px-3">
                        <div>
                            ...
                            <input type="text" id="telepon" name="telepon" value="<?php echo htmlspecialchars($santri['telepon']); ?>" class="border border-gray-300 rounded-lg px-4 py-2 w-full text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                        </div>
                        <div>
                            <label for="email" class="block text-gray-700 text-sm font-medium mb-2">Email</label>
                            <input type="email" id="email" name="email" value="<?php echo htmlspecialchars($santri['email']); ?>" class="border border-gray-300 rounded-lg px-4 py-2 w-full text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                        </div>
                        <div>
                            <label for="tanggal_lahir" class="block text-gray-700 text-sm font-medium mb-2">Tanggal Lahir</label>
                            <input type="date" id="tanggal_lahir" name="tanggal_lahir" value="<?php echo htmlspecialchars($santri['tanggal_lahir']); ?>" class="border border-gray-300 rounded-lg px-4 py-2 w-full text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                        </div>
                    </div>
